...
const schema = z.object({
  ...
  SEND_INSTALLATION_EMAIL: z.enum(["true", "false"]).default("true"),
  CREATE_QUOTE_ON_SUBMIT: z.enum(["true", "false"]).default("true"),
  ENABLE_AUTO_SCHEDULING: z.enum(["true", "false"]).default("false"),
  REQUIRE_SURVEYOR_AUTH: z.enum(["true", "false"]).default("true"),
  SURVEYOR_ACCESS_MODE: z.enum(["assigned", "tenant"]).default("tenant"),
  AZURE_TENANT_ID: z.string().default(""),
  ENABLE_NEW_PRODUCT_REQUESTS: z.enum(["true", "false"]).default("false"),
  ENABLE_DATAVERSE_SURVEY_LAYOUT: z.enum(["true", "false"]).default("false"),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    sendInstallationEmail: value.SEND_INSTALLATION_EMAIL === "true",
    createQuoteOnSubmit: value.CREATE_QUOTE_ON_SUBMIT === "true",
    enableAutoScheduling: value.ENABLE_AUTO_SCHEDULING === "true",
    requireSurveyorAuth: value.REQUIRE_SURVEYOR_AUTH === "true",
    surveyorAccessMode: value.SURVEYOR_ACCESS_MODE,
    azureTenantId: value.AZURE_TENANT_ID.trim().toLowerCase(),
    enableNewProductRequests: value.ENABLE_NEW_PRODUCT_REQUESTS === "true",
    enableDataverseSurveyLayout: value.ENABLE_DATAVERSE_SURVEY_LAYOUT === "true",
