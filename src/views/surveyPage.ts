...
export function renderSurveyForm(model: SurveyFormViewModel): string {
  ...
  return page("Access4Lofts Survey", `<form method="post" action="/api/survey/${encodeURIComponent(model.token)}" id="survey-form">
    <header class="app-header"><div class="header-inner">${logo()}<div><strong>Access4Lofts ${escapeHtml(franchise)}</strong><span>${escapeHtml(context.region.name)}</span></div></div></header>
    <main class="page-shell">
      <article class="form-sheet">
        ...
        <section class="submit-section"><div><h2>Review and submit</h2><p><strong><span id="selected-product-count">0</span> products selected.</strong> Only these products will be placed on the Opportunity and draft Quote.</p></div><button class="primary" type="submit">Complete survey and create quote</button></section>
      </article>
    </main>
  </form>`);
}
...
export function renderSurveyThanks(productCount: number, requestedProductCount = 0, quoteId?: string): string {
  const outcome = requestedProductCount
    ...
    : quoteId ? "A draft quote containing only the selected products has been created for office review." : "The office can now continue the quotation process.";
  return page("Survey saved", `<main class="confirmation"><div class="brand-small">${logo()}</div><div class="success-mark">✓</div><h1>Survey completed</h1><p>${productCount} selected product line${productCount === 1 ? " was" : "s were"} saved.</p><p>${outcome}</p><p>You can close this window.</p></main>`);
}
...
function productRow(product: ProductOption): string {
  ...
  return `<article class="product-row${selected ? " is-selected" : ""}" data-unit-price="${product.price}">
